...
public class SymbolicNewObjectsCollector implements ExecutionVisitor
{
	// map from program points to symbolic expressions that represent newly allocated objects (at the respective location)
	protected static Map<ProgramPoint, NewObjectExpression> pp2NewObj;
	
	static
	{
		pp2NewObj = new LinkedHashMap<ProgramPoint, NewObjectExpression>();
	}
	
	
	public static void analyzeProgram(StaticAnalysisContext staCtx, List<String> methodSigPrefixes) throws Exception
	{
		// for each program point that corresponds to new object allocation, we gather the symbolic expression that represents the new object
		
		SymbolicNewObjectsCollector objCollector = new SymbolicNewObjectsCollector();
		
		SymbolicByteCodeInterpreter.processReachableMethods(staCtx, objCollector, methodSigPrefixes);  
	}
	
	public static NewObjectExpression getNewObjectForProgramPoint(ProgramPoint pp)
	{
		return pp2NewObj.get(pp);
	}
	
	public static void printNewObjects()
	{
		System.out.println("NEW OBJECTS");
		System.out.println("===========");
		
		Set<ProgramPoint> progPoints = new TreeSet<ProgramPoint>();
		progPoints.addAll(pp2NewObj.keySet());
		
		for (ProgramPoint pp : progPoints)
		{
			if (Utils.isJavaStandardLibraryMethod(pp.methodSig)) continue;
	
			NewObjectExpression newObj = pp2NewObj.get(pp);
